...
{% hint style="danger" %}
사용자 스크립트 실행 중 논리 오류 또는 조건 미충족 시 로봇이 예기치 않게 동작하여 중상 또는 사망에 이를 수 있습니다.
{% endhint %}
...
# 3.4.5. `switch`~`case`~`break`~`end_switch`문

### 설명

`switch` 문은 수식을 평가하여 `case`문으로 지정한 수식을 평가한 값과 비교합니다. 값이 일치하는 `case`문부터 `break` 문을 만날 때까지 수행합니다. 

예를 들어, 아래에서 표현식 X의 결과값이 표현식 B1이나 B2의 결과값와 같다면, \(1\) ~ \(3\)이 수행한 후 `end_switch` 문 지점으로 이동합니다. \(명령문 B 밑에 `break`가 없는 점을 유의하십시오.\) X의 값이 C와 같을 때는 \(2\) ~ \(3\)이 수행됩니다. 

{% hint style="warning" %}
`break`문이 없으면 다음 `case`문의 명령문들까지 수행됩니다. Hi5a의 HR-BASIC 문법의 `CASE`문에 익숙한 분들은 주의 바랍니다.
{% endhint %}

만일, X의 값이 어떤 `case` 문의 값과도 일치하지 않으면, `default`로 이동하여 \(4\) ~ \(5\)가 수행됩니다. default 구간은 없어도 됩니다.
...
### 전송 데이터
전송하는 데이터는 기본 태그에 HRScript 명령문 실행에 의해 옵션 태그를 추가할 수 있습니다. <br>
태그는 하기와 같이 구성됩니다. <br>
<기본 태그>
- cur_po : 로봇의 위치와 자세에 대한 현재값입니다. (x, y, z, rx, ry, rz) 
...
- trigger : rsi.on 실행 후 0의 값으로 자동 생성되는 태그입니다. <br>
rsi.put("trigger", 1) 실행하여 값을 1로 변경할 수 있으며 센서 장치에서 값을 읽어 처리하는 로직을 구성할 수 있습니다. <br>

<옵션 태그> 
- cmd_po : 로봇의 위치와 자세에 대한 지령값입니다. (x, y, z, rx, ry, rz) <br>
rsi.put("cmd_po") 실행하면 "cmd_po" 태그가 "cur_po" 태그와 동일한 형식으로 추가됩니다. 
- 사용자 태그 : rsi.put("태그 이름", 값) 실행에 의해 사용자 태그를 추가하고 이 태그의 값을 변경할 수 있습니다. <br>
<br>
<br>

#### 전송 데이터는 문서 형식에 따라 하기와 같습니다.
rsi.format="json" 또는 rsi.format="xml"의 실행에 의해 문서 형식을 "JSON" 또는 "XML"로 지정할 수 있습니다. (기본값 = "JSON")
<br>
...
&lt;RSI객체&gt;.put("cmd_po") <br>
&lt;RSI객체&gt;.put("trigger", 1) <br>

### 리턴값
- 1: 기존 태그에 값을 변경
- 0: 옵션 태그를 추가
- -1: 함수의 인자가 1개만 존재할 때, 입력된 인자가 시스템 태그가 아님
...
{% hint style="warning" %}
* 제어기 AS 요원 및 개발자용 진단 명령문입니다. 정상 가동 중엔 사용하지 마십시오.
...
어떤 데이터를 어떤 형식으로 수집할 것인지는 서비스 - 데이터 수집 화면에서 미리 설정해둬야 합니다.
...
{% hint style="warning" %}
* **triggout 명령어가 존재하는 스텝**을 기준으로, **다음 스텝이 종료되는 시점까지** 신호 출력 여부를 검사합니다. 
* 해당 종료 시점 **이내에 신호가 출력되지 않을 경우**,  
  **W0241 : _"triggout 신호가 스텝 범위 내에 출력되지 않았습니다"_** 경고를 표시합니다.
...
- 압력을 추정하기 위해 가스스프링이 장착된 축을 현재 위치에서 -20도만큼 왕복 운전합니다.(H축 140도 자세에서 수행 권장)
...
현재 선택된 툴의 번호를 얻거나 툴데이터를 읽거나 변경하기 위한 시스템 변수입니다.   
...
```python
   print _tool_no # 현재 선택된 툴의 번호를 출력
